...
export function productsForCollection(products: Product[], slug: string): Product[] {
  ...
  if (slug === "wedding-edit") return products.filter((product) => product.occasions?.some((occasion) => occasion.toLowerCase() === "wedding"));
  if (slug === "festive-wear") return products.filter((product) => product.occasions?.some((occasion) => ["festive", "occasion"].includes(occasion.toLowerCase())));
  return [];
}
