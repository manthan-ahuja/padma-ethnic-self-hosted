...
export function CartPageContent({ products }: { products: Product[] }) {
  ...
  const startCheckout = async () => {
    ...
    setCheckoutError("");
    try {
      const response = await fetch("/api/shopify/cart", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lines: cart.items.map((item) => ({ merchandiseId: item.selection?.variantId, quantity: item.quantity })) }) });
  ...
  return <main className="full-cart-page">
    ...
    {cart.items.length === 0 ? <div className="cart-empty-page"><h2>Your bag is waiting</h2><p>Discover pieces designed for celebrations and every day between them.</p><Link className="primary-cta" href="/collections/all">Explore the collection <ArrowRight size={15} /></Link></div> : <div className="cart-page-grid">
      ...
      <aside className="order-summary"><p className="eyebrow">Order summary</p><div><span>Subtotal</span><strong>{money.format(cart.subtotal)}</strong></div><div><span>Delivery</span><span>Calculated at checkout</span></div><label>Discount code<input placeholder="Enter code" disabled title="Discount codes can be entered at Shopify checkout" /></label><button type="button" className="checkout-button" disabled={!checkoutReady || checkoutPending} onClick={startCheckout}>{checkoutPending ? "Opening secure checkout…" : checkoutReady ? "Secure checkout" : "Checkout opens after Shopify connection"} <ArrowRight size={16} /></button>{checkoutError && <p className="selection-message is-error" role="alert">{checkoutError}</p>}<p className="integration-note"><ShieldCheck size={16} /> {checkoutReady ? "Live inventory and totals are revalidated by Shopify before payment." : "This preview bag contains local products. Connect Shopify, then re-add products to enable secure checkout."}</p></aside>
    </div>}
