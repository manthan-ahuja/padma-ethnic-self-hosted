...
export function ProductDetail({ product }: { product: Product }) {
  ...
  const shareProduct = async () => {
    ...
    else if (navigator.clipboard) { await navigator.clipboard.writeText(window.location.href); setConfirmation("Product link copied."); }
  };
  ...
  return <>
    <main className="product-detail" aria-labelledby="product-title">
      <section className={`product-gallery ${product.gallery.length === 1 ? "single-image" : ""}`} aria-label={`${product.name} image gallery`}>
        ...
        <button type="button" className="product-gallery-main" onClick={() => setZoomOpen(true)} aria-label={`Open full-screen view of ${product.name}`}>
      ...
      <section className="product-detail-copy">
        ...
        <div className="product-detail-price"><span>{money.format(product.price)}</span>{product.originalPrice && <del>{money.format(product.originalPrice)}</del>}<small>Inclusive of taxes</small></div>
        ...
        <fieldset className="product-options colour-options"><legend>Colour <span>{selectedColor}</span></legend><div className="product-option-row">{product.colors.map((color) => <label key={color} className={selectedColor === color ? "is-selected" : ""}><input type="radio" name="colour" value={color} checked={selectedColor === color} onChange={() => setSelectedColor(color)} /><span className={`detail-swatch swatch-${color.toLowerCase().replaceAll(" ", "-")}`} />{color}</label>)}</div></fieldset>
        <fieldset className="product-options size-options"><legend>Choose size <a href="#size-guide">Size guide</a></legend><div className="product-option-row">{product.sizes.map((size) => <label key={size} className={selectedSize === size ? "is-selected" : ""}><input type="radio" name="size" value={size} checked={selectedSize === size} onChange={() => { setSelectedSize(size); setSelectionError(""); }} />{size}</label>)}</div></fieldset>
        ...
        <div className="purchase-assurances"><span><Truck size={18} /><strong>Complimentary delivery</strong> across India</span><span><RotateCcw size={18} /><strong>Easy 7-day returns</strong> on eligible pieces</span><span><ShieldCheck size={18} /><strong>Secure checkout</strong> activates with the commerce backend</span></div>
        <div className="product-information"><details open><summary>Features <Plus size={16} /></summary><ul>{product.features.map((feature) => <li key={feature}>{feature}</li>)}</ul></details><details><summary>Fit &amp; measurements <Plus size={16} /></summary><p>{product.measurements ?? "Detailed garment measurements will be published alongside the final production size chart."}</p></details><details><summary>Material &amp; care <Plus size={16} /></summary><p>{product.care}</p></details><details><summary>What&apos;s included <Plus size={16} /></summary><p>{product.included}</p></details><details><summary>Craft &amp; product details <Plus size={16} /></summary><p>{product.origin}</p><p>Product code: {product.sku}</p></details></div>
      </section>
    </main>
    <div className="mobile-purchase-bar"><div><strong>{product.name}</strong><span>{money.format(product.price)}</span></div><button type="button" aria-label="Add to bag from sticky bar" onClick={addConfiguredProduct}>Add to bag</button></div>
    {zoomOpen && <div className="gallery-lightbox" role="dialog" aria-modal="true" aria-label={`${product.name} full-screen gallery`}><button type="button" onClick={() => setZoomOpen(false)} aria-label="Close full-screen gallery"><X /></button><div><Image src={selectedImage.src} alt={selectedImage.alt} fill sizes="100vw" style={{ objectPosition: selectedImage.position }} /></div><p>{activeImage + 1} / {product.gallery.length}</p></div>}
  </>;
}
